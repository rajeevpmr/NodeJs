...
function recordNorFound(message){
    Error.call(this);
    ...
    function(err, rows, fields){
        if(!err) {
            if (rows.length > 0) {
                ...
                callback(new recordNorFound('Record does not exist'),
    ...
    function(err, rows, fields){
        if(!err) {
            if (rows.length > 0) {
                ...
                callback(new recordNorFound('No Records exist'),
...
Contacts.prototype.save = function(contact, callback){
    try {
        ...
        connection.query(
            'UPDATE contact_person set firstName=?, lastname=?, email=?, phone=?, imagePath=?' +
            ' where contactId=?',
